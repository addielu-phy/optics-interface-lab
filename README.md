...
**六界面獨立操作頁：** https://addielu-phy.github.io/optics-interface-lab/six-interface.html
...
- 六界面另有獨立操作頁，將入射角控制與光路圖上下緊鄰，方便學生一邊拉動角度、一邊觀察光路；方向箭頭採固定視覺尺寸，不會隨線寬過度放大。
...
- 獨立的筆電／平板版固定保留控制台與光路圖左右並列，可直接按住入射光線或光源拖曳角度；界面 A 入射點與七層介質比例保持固定，每段光路都有固定尺寸的方向箭頭；介質以折射率連續色階呈現，數值越大顏色越深。
...
瀏覽器 QA 會同時檢查完整首頁與六界面獨立頁的五個精確視窗寬度、水平溢位、觸控目標、SVG 教學文字、方向箭頭比例、角度控制與光路圖順序、WCAG A/AA、實際操作流程、無效輸入保留最後有效狀態，以及瀏覽器錯誤。Windows 預設使用已安裝的 Microsoft Edge；其他平台可先執行：
